...
# 排除重複數據
df = df.drop_duplicates(subset=['Flight', 'STD', 'STA', 'Registration', 'Irreg State'])
...
# 排除機坪滑回（Irreg State 欄為 RTR 的列）
num_RTR = len(df[df['Irreg State'] == 'RTR'])
df = df[df['Irreg State'] != 'RTR']

# 排除訓練航班（Service Type 為 K）
num_K = len(df[df['Service Type'] == 'K'])
df = df[df['Service Type'] != 'K']

# 排除客運包機（Service Type 為 C）
num_C = len(df[df['Service Type'] == 'C'])
df = df[df['Service Type'] != 'C']

# 排除空機飛渡（Service Type 為 P）
num_P = len(df[df['Service Type'] == 'P'])
df = df[df['Service Type'] != 'P']
...
print(f"機坪滑回架次: {num_RTR}")
print(f"訓練航班架次: {num_K}")
print(f"客運包機架次: {num_C}")
print(f"空機飛渡架次: {num_P}")
print(f"\n=== 定期航班 ===")
print(f"旅客航班架次: {tot_FLT}")
print(f"載運旅客總量: {tot_PAX}")
...
def otp_calc(df_intrest, otp_type=None):
    ...
    # 顯示結果
    if otp_type is None or otp_type == 0:
        print(f"\n=== 所有延誤 ===")
        print(f"延誤起飛共 {num_DLY} 架次 ({round(100*num_DLY/len(df_intrest), 2)}%)")
        print(f"平均每航班延誤 {DLYT_avg} 分鐘")
        print(f"全時段總延誤 {DLYT_ttl} 小時")
        print(f"受影響乘客共 {DLYP_ttl} 人次")
        print(f"\n=== 可控延誤 ===")
        print(f"延誤起飛共 {num_DLY_Ctl} 架次 ({round(100*num_DLY_Ctl/len(df_intrest), 2)}%)")
        print(f"受影響乘客共 {DLYP_Ctl_ttl} 人次")
        print(f"可控之準時率為 {round(100 - 100*num_DLY_Ctl/len(df_intrest), 2)}%\n")
...
if run_ID[4] == 1:
    ...
    OT_hour = pd.concat([OT_hour, OT_df], axis=1)
		...
		# 將 DLYC 進行排序
    DLYC_CNT_idx = OT_hour.columns.get_loc('DLY Cnt')
    sorted_cols = sorted(OT_hour.columns[DLYC_CNT_idx + 1:])
    new_cols = list(OT_hour.columns[:DLYC_CNT_idx + 1]) + sorted_cols
    OT_hour = OT_hour[new_cols]
    ...
    for i, row in valid_OT_hour.iterrows():
        plt.text(row['Period'] + 0.5, row['TTL FLT'] + 0.02*valid_OT_hour['TTL FLT'].max(), f"{row['Ctl OTP']:.3g}%", ha='center', va='bottom')
...
if run_ID[3] == 1:
    ...
    OT_day = pd.concat([OT_day, OT_df], axis=1)
		...
		# 將 DLYC 進行排序
    DLYC_CNT_idx = OT_day.columns.get_loc('DLY Cnt')
    sorted_cols = sorted(OT_day.columns[DLYC_CNT_idx + 1:])
    new_cols = list(OT_day.columns[:DLYC_CNT_idx + 1]) + sorted_cols
    OT_day = OT_day[new_cols]
...
if run_ID[2] == 1:
    ...
    OT_week = pd.concat([OT_week, OT_df], axis=1)
		...
		# 將 DLYC 進行排序
    DLYC_CNT_idx = OT_week.columns.get_loc('DLY Cnt')
    sorted_cols = sorted(OT_week.columns[DLYC_CNT_idx + 1:])
    new_cols = list(OT_week.columns[:DLYC_CNT_idx + 1]) + sorted_cols
    OT_week = OT_week[new_cols]
    ...
    if x_itv <= 2:
        ...
        plt.plot(date2num(OT_week['Period']), np.array(OT_week['Ctl OTP']), marker='o', label='On-time Percentage')

        # 註記準時率
        for i, row in valid_OT_week.iterrows():
            plt.text(date2num(valid_OT_week['Period'])[i], row['OTP'] - 3, f"{row['OTP']:.3g}%", ha='center', va='top')

        # 註記可控準時率
        for i, row in valid_OT_week.iterrows():
            plt.text(date2num(valid_OT_week['Period'])[i], row['Ctl OTP'] + 3, f"{row['Ctl OTP']:.3g}%", ha='center', va='bottom')

        # 註記航班總數
        for i, row in valid_OT_week.iterrows():
            plt.text(date2num(valid_OT_week['Period'])[i], 3, f"{int(row['TTL FLT'])}", ha='center', va='bottom')
...
if run_ID[1] == 1:
    ...
    OT_month = pd.concat([OT_month, OT_df], axis=1)
		...
		# 將 DLYC 進行排序
    DLYC_CNT_idx = OT_month.columns.get_loc('DLY Cnt')
    sorted_cols = sorted(OT_month.columns[DLYC_CNT_idx + 1:])
    new_cols = list(OT_month.columns[:DLYC_CNT_idx + 1]) + sorted_cols
    OT_month = OT_month[new_cols]
    ...
    if x_itv >= 4:
        ...
        plt.plot(date2num(valid_OT_month['Period']), np.array(valid_OT_month['Ctl OTP']), marker='o', label='On-time Percentage')
        ...
        for i, row in valid_OT_month.iterrows():
            if row['TTL FLT'] != 0:
                plt.text(date2num(row['Period']), row['OTP'] - 3, f"{row['OTP']:.3g}%", ha='center', va='top')

        # 註記可控準時率
        for i, row in valid_OT_month.iterrows():
            if row['TTL FLT'] != 0:
                plt.text(date2num(row['Period']), row['Ctl OTP'] + 3, f"{row['Ctl OTP']:.3g}%", ha='center', va='bottom')

        # 註記航班數量
        for i, row in valid_OT_month.iterrows():
            if row['TTL FLT'] != 0:
                plt.text(date2num(row['Period']), 3, f"{int(row['TTL FLT'])}", ha='center', va='bottom')
...
def otp_apt(APT=None):
    all_APT = sorted(df['Arr'].unique())
    all_APT.remove(base)
    if APT is None or APT == ['']:
        APT = all_APT

    for airport in APT:
        df_APT = df_base[df_base['Arr'].isin([airport])]
        OT_APT = pd.DataFrame({'Arr': [airport]})
        if airport in all_APT:
            if len(APT) == 1:
                print(f'{base} 飛往 {airport} 之延誤狀況')
                OT_APT = pd.concat([OT_APT, otp_calc(df_APT)], axis=1)
            else:
                OT_APT = pd.concat([OT_APT, otp_calc(df_APT, -1)], axis=1)
            if 'OT_APT_All' not in locals():
                OT_APT_All = OT_APT
            else:
                OT_APT_All = pd.concat([OT_APT_All, OT_APT], axis=0)
        else:
            print(f'時段內並無航班飛往 {airport} 機場！')

    if 'OT_APT_All' in locals():
        # 將 DLYC 進行排序
        DLYC_CNT_idx = OT_APT_All.columns.get_loc('DLY Cnt')
        sorted_cols = sorted(OT_APT_All.columns[DLYC_CNT_idx + 1:])
        new_cols = list(OT_APT_All.columns[:DLYC_CNT_idx + 1]) + sorted_cols
        OT_APT_All = OT_APT_All[new_cols]

        # 輸出準時率表單
        if file_ID == 1:
            save_path = os.path.join(outFolder, f"{base}_OTP-APT_{date_start.strftime('%Y%m%d')}-{date_end.strftime('%Y%m%d')}.csv")
            OT_APT_All[pd.notna(OT_APT_All['OTP'])].to_csv(save_path, index=False)
            print(f"\n各航點準時率表單輸出完成！")
